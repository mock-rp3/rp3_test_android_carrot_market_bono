...
- 검색화면 구현 **( 80% ✔)**
  ...
  - 프래그먼트에 데이터를 넘겨줄 때, intent는 액티비티간의 이동이 필요한거 같아서 SharedPreferences를 사용하여 넘겨줬다. 넘겨준 후에는 초기화를 진행하였다.
  - 프래그먼트 별 검색은 TabLayoutMediator에서 position == 0 일때, position == 2 일때, 등등 나눠서 코드를 작성해주었다.
